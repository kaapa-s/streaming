#!/bin/sh
# Point /etc/nginx/ssl at Let's Encrypt when issued; otherwise self-signed.
# Keeps bootstrap certs out of /etc/letsencrypt so certbot is not confused.
set -e

if [ -z "${SERVER_NAME}" ]; then
  echo "SERVER_NAME is required (sslip hostname, e.g. 203-0-113-42.sslip.io)" >&2
  exit 1
fi
